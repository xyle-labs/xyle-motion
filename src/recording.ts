...
import { z } from 'zod';
import { enhanceRecording, wavSeconds, LOCAL_SPEAKERS, neuralVoiceAvailable } from './audio.ts';
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname === '/api/take') {
        ...
        const decoded = spawnSync('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-nostdin', '-i', original, '-vn', '-t', '600', '-ar', '48000', '-ac', '1', '-c:a', 'pcm_s16le', raw], { encoding: 'utf8', timeout: 120_000 });
        if (decoded.error || decoded.status !== 0) throw new Error(`Could not decode the take. Full ffmpeg must be installed. ${decoded.error?.message ?? decoded.stderr.slice(-500)}`);
        const seconds = wavSeconds(raw);
        if (!seconds || seconds >= 600) throw new Error('Record one scene at a time (under 10 minutes). Original file preserved.');
